...
def standardize_campaign_columns(df):
    ...
    # Normalize delivery status values to "Active" or "Inactive"
...
if shopify_files:
    ...
    # Check if the Shopify product name CONTAINS the campaign product name
...
def convert_final_campaign_to_excel_staff_with_date_columns_fixed(df, shopify_df=None, selected_days=None):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        if shopify_files:  # Use the RAW uploaded files directly
            ...
            # FUZZY MATCHING: Create a mapping from Campaign product names to Shopify product names
            # Using THE EXACT SAME fuzzy matching approach as your existing campaign processing
            # IMPROVED FUZZY MATCHING: Create a mapping from Campaign product names to Shopify product names
            ...
            if df_campaign is not None and 'Canonical Product' in df_campaign.columns:
                ...
                st.info(f"🔍 Starting improved matching for {len(campaign_products)} campaign products against {len(shopify_raw_product_names)} Shopify products...")
                ...
                for campaign_product in campaign_products:
                    ...
                    # STEP 1: Try SUBSTRING matching first (most reliable)
                    matched = False
                    for shopify_name in shopify_raw_product_names:
                        shopify_name_lower = shopify_name.lower().strip()
                        
                        # If campaign name is contained in Shopify name, it's a match!
                        if campaign_product_lower in shopify_name_lower:
                            shopify_product_fuzzy_mapping[campaign_product_clean] = shopify_name
                            st.success(f"✅ Substring Match: '{campaign_product_clean}' → '{shopify_name}'")
                            matched = True
                            break
                        
                        # Also check reverse: if Shopify name is contained in campaign name
                        if shopify_name_lower in campaign_product_lower:
                            shopify_product_fuzzy_mapping[campaign_product_clean] = shopify_name
                            st.success(f"✅ Reverse Substring Match: '{campaign_product_clean}' → '{shopify_name}'")
                            matched = True
                            break
                    
                    # STEP 2: If no substring match, try fuzzy matching
                    if not matched:
                        shopify_names_lower = [name.lower() for name in shopify_raw_product_names]
                        
                        result = process.extractOne(
                            campaign_product_lower,
                            shopify_names_lower,
                        ...
                        if result:
                            ...
                            st.success(f"✅ Fuzzy Match ({match_score}%): '{campaign_product_clean}' → '{matched_shopify_product}'")
                        else:
...
# DEBUG: Show lookups
            ...
            for campaign_info in campaign_cpp_list:
                ...
                for date in unique_dates:
                    ...
                    if not date_data.empty:
                        ...
                        # No data for this date
                        safe_write(worksheet, campaign_row_idx, delivery_status_col_idx, "", campaign_format)
                ...
                for metric in date_metrics:
                    total_col_idx = all_columns.index(f"Total_{metric}")
                    if metric == "Delivery status":
                        # For Delivery status, just leave it empty in totals (not applicable)
                        safe_write(worksheet, campaign_row_idx, total_col_idx, "", campaign_format)
                    
                    elif metric in ["Avg Price", "Delivery Rate"]:
            ...
            if campaign_rows:
                ...
                for date in unique_dates:
                    for metric in date_metrics:
                        ...
                        if metric == "Delivery status":
                            # For Delivery status in product totals, leave empty (not applicable)
                            safe_write(worksheet, product_total_row_idx, col_idx, "", product_total_format)
        ...
        if product_total_rows:
            ...
            for date in unique_dates:
                for metric in date_metrics:
                    ...
                    if metric == "Delivery status":
                        # For Delivery status in grand total, leave empty (not applicable)
                        safe_write(worksheet, grand_total_row_idx, col_idx, "", grand_total_format)
            ...
            for metric in date_metrics:
                ...
                if metric == "Delivery status":
                    ...
                    total_avg_price_idx = round(all_columns.index("Total_Avg Price"),2)
                    total_purchases_idx = round(all_columns.index("Total_Purchases"),2)
                    total_amount_spent_idx = round(all_columns.index("Total_Amount Spent (USD)"),2)
                    total_delivery_rate_idx = round(all_columns.index("Total_Delivery Rate"),2)
        ...
        unmatched_headers = ["Status", "Product", "Campaign Name", "Amount Spent (USD)", 
                            "Purchases", "Cost Per Purchase (USD)", "Last Day Delivery Status", "Dates Covered", "Reason"]
        ...
        if unmatched_campaigns:
            ...
            for campaign in unmatched_campaigns:
                ...
                safe_write(unmatched_sheet, current_row, 6, last_day_delivery_status, unmatched_data_format)
        ...
        # ==== SHEET: Negative Score Campaigns WITH COMPLETE ANALYSIS TABLE ====
        ...
        safe_write(negative_score_sheet, current_row, 0, 
                  "COMPLETE CAMPAIGN ANALYSIS - ALL VALID PRODUCTS", 
                  analysis_header_format)
        current_row += 2
        
        # Build headers - Product, Campaign, Day-wise columns for each date, Total Score, CPP, BE
        analysis_headers = ["Product", "Campaign Name"]
        
        # Add separator after Campaign Name
        analysis_headers.append("SEPARATOR_AFTER_CAMPAIGN")
        
        # Add day-wise columns for each date (6 visible metrics + 1 hidden) WITH SEPARATORS
        for date in unique_dates:
            analysis_headers.extend([
                ...
                f"{date} Purchases",
                f"{date} Avg Price",
                f"{date} Product Cost Input",  # HIDDEN COLUMN
        ...
        # Add summary columns
        analysis_headers.extend(["Total Score", "C.P.P", "B.E"])
        
        # Write headers (skip separator columns and hide Product Cost Input columns)
        col_num = 0
        for header in analysis_headers:
            if header.startswith("SEPARATOR_"):
                col_num += 1
                continue
            elif "Product Cost Input" in header:
                # Write header but it will be hidden
                safe_write(negative_score_sheet, current_row, col_num, header, analysis_header_format)
                col_num += 1
            else:
                safe_write(negative_score_sheet, current_row, col_num, header, analysis_header_format)
                col_num += 1
        ...
        for product, product_df in valid_products:
            for campaign_name, campaign_group in product_df.groupby("Campaign Name"):
                ...
                # Get BE value (calculated at product level)
                product_costs = []
                for date in unique_dates:
                    date_cost = product_date_cost_inputs.get(product, {}).get(date, 0)
                    if date_cost > 0:
                ...
                for date in unique_dates:
                    ...
                    if not date_data.empty:
                        ...
                        if date_avg_price > 0 and (date_purchases > 0 or 
                            ...
                            # Score = Net Profit / (Revenue * 0.1)
                            # Net Profit = Revenue - Costs
                            # Revenue = Avg Price * Delivered Orders
                            # Costs = Amount Spent * 100 + Shipping + Operational + Product Cost
                            
                            
                            # Revenue calculation - use actual purchases
                            revenue = date_avg_price * date_purchases * delivery_rate
                            
                            # Costs calculation - use actual purchases
                            costs = (date_amount_spent * 100) + (77 * date_purchases) + (65 * date_purchases) + (date_product_cost * date_purchases * delivery_rate)
                            
                            # Net Profit
                            net_profit = revenue - costs
                            
                            # Score = Net Profit / (Revenue * 0.1)
                            # FIXED: For denominator, use MAX(purchases, 1) ONLY when amount_spent > 0
                            purchases_for_denominator = max(date_purchases, 1) if date_amount_spent > 0 else date_purchases
                            denominator = date_avg_price * purchases_for_denominator * delivery_rate * 0.1
                            
                            day_score = round((net_profit / denominator), 2) if denominator > 0 else 0
                ...
                # CALCULATE TOTAL SCORE (day-by-day sum method matching admin's net profit % calculation)
                total_score = 0
                total_net_profit_sum = 0
                
                if product_avg_price > 0:
                    # Calculate net profit by summing day-by-day
                    for date in campaign_dates:
                        ...
                        if not date_data.empty:
                            ...
                            total_shipping_cost_date = round(calc_purchases_date * 77, 2)
                            total_operational_cost_date = round(calc_purchases_date * 65, 2)
                    ...
                    # Calculate Total Score = Net Profit % / 10
                    ...
                    net_profit_percentage = round((numerator_total / denominator_total * 100), 2) \
                        if denominator_total > 0 else 0
                    
                    total_score = round(net_profit_percentage / 10, 2)
                else:
                    total_score = 0
        ...
        for campaign_data in all_campaigns_complete_analysis:
            ...
            safe_write(negative_score_sheet, current_row, col_num, 
            ...
            for date in unique_dates:
                ...
                # Write all 7 metrics for this date (6 visible + 1 hidden)
                safe_write(negative_score_sheet, current_row, col_num, 
                ...
                col_num += 1
                safe_write(negative_score_sheet, current_row, col_num, 
                          day_metrics.get('score', 0), analysis_data_format)
                col_num += 1
            ...
            safe_write(negative_score_sheet, current_row, col_num, 
                      campaign_data['Total Score'], analysis_data_format)
            col_num += 1
            safe_write(negative_score_sheet, current_row, col_num, 
                      campaign_data['C.P.P'], analysis_data_format)
            col_num += 1
            safe_write(negative_score_sheet, current_row, col_num, 
                      campaign_data['B.E'], analysis_data_format)
        ...
        safe_write(negative_score_sheet, current_row, 0, 
                  ...
                  analysis_header_format)
        safe_write(negative_score_sheet, current_row + 1, 0, 
                  f"TOTAL UNIQUE DATES: {len(unique_dates)}", 
                  analysis_header_format)
        safe_write(negative_score_sheet, current_row + 2, 0, 
       ...
       # HIDE THE COMPLETE ANALYSIS TABLE (rows 0 to current_row - 6)
        analysis_table_end_row = current_row - 6
        for row_idx in range(0, analysis_table_end_row + 1):
            negative_score_sheet.set_row(row_idx, None, None, {'hidden': True})
        
        # Add a visible header for the filtered tables section
        safe_write(negative_score_sheet, current_row, 0, 
                  "FILTERED CAMPAIGN ANALYSIS TABLES", 
                  severe_negative_header_format)
        current_row += 2
        
        # ==== NOW CONTINUE WITH FILTERED TABLES ====
        ...
        for campaign_complete in all_campaigns_complete_analysis:
            ...
            if negative_days_count >= selected_days:
                ...
                campaign_analysis = {
                    ...
                    'Days with Negative Score': negative_days_count,
                    'C.P.P': campaign_complete['C.P.P'],
                    'B.E': campaign_complete['B.E'],
                    'Amount Spent (USD)': round(total_amount_spent_usd, 2),
                    'Total Purchases': int(total_purchases),
                    'Total Score': campaign_complete['Total Score'],
                    'Last Day Delivery Status': last_day_delivery_status,
                    'Negative Score Dates': formatted_dates_str,
                    'Reason': f"Has {negative_days_count} negative score days out of {total_dates} total days (threshold: {selected_days})"
                }
        ...
        # STEP 3: Split into categories based on Total Score (STAFF THRESHOLDS - CORRECTED)
        # Table 1: Score ≤ -1 (includes -10, -15, etc.)
        severe_negative_campaigns = [c for c in filtered_campaigns if c['Total Score'] <= -1]
        # Table 2: -1 < Score < 0 (between -1 and 0, excluding both)
        moderate_negative_campaigns = [c for c in filtered_campaigns 
                                      if -1 < c['Total Score'] < 0]
        # Table 3: Score ≥ 0 (0 and above)
        positive_campaigns = [c for c in filtered_campaigns if c['Total Score'] >= 0]

        # Sort all groups
        severe_negative_campaigns.sort(key=lambda x: x['Total Score'])
        moderate_negative_campaigns.sort(key=lambda x: x['Total Score'])
        positive_campaigns.sort(key=lambda x: x['Total Score'], reverse=True)

        # STEP 4: Write to Excel - TABLE 1: SEVERE NEGATIVE (Score ≤ -1)
        safe_write(negative_score_sheet, current_row, 0, 
                  "CAMPAIGNS WITH SEVERE NEGATIVE SCORE (SCORE ≤ -1)", 
                  severe_negative_header_format)
        current_row += 1

        severe_headers = ["Product", "Campaign Name", "C.P.P", "B.E", "Amount Spent (USD)", 
                         "Score", "Last Day Delivery Status", "Comment", "Total Dates", 
                         "Days Checked", "Days with Negative Score", 
                         "Negative Score Dates", "Reason"]

        for col_num, header in enumerate(severe_headers):
            safe_write(negative_score_sheet, current_row, col_num, header, 
                      severe_negative_header_format)
        current_row += 1
        ...
        if severe_negative_campaigns:
            for campaign in severe_negative_campaigns:
                ...
                safe_write(negative_score_sheet, current_row, 0, campaign['Product'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 1, campaign['Campaign Name'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 2, campaign['C.P.P'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 3, campaign['B.E'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 4, campaign['Amount Spent (USD)'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 5, campaign['Total Score'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 6, campaign['Last Day Delivery Status'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 7, comment, 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 8, campaign['Total Dates'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 9, campaign['Days Checked'], 
                          severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 10, 
                          campaign['Days with Negative Score'], severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 11, 
                          campaign['Negative Score Dates'], severe_negative_data_format)
                safe_write(negative_score_sheet, current_row, 12, campaign['Reason'], 
                          severe_negative_data_format)
                current_row += 1
        else:
            safe_write(negative_score_sheet, current_row, 0, 
                      "No campaigns found with severe negative score (Score ≤ -1)", 
                      severe_negative_data_format)
            current_row += 1

        # Summary for severe
        current_row += 2
        safe_write(negative_score_sheet, current_row, 0, "SUMMARY - SEVERE NEGATIVE CAMPAIGNS", 
                  severe_negative_header_format)
        safe_write(negative_score_sheet, current_row + 1, 0, 
                  f"Campaigns with severe negative score (Score ≤ -1): {len(severe_negative_campaigns)}", 
                  severe_negative_data_format)
        current_row += 3

        # TABLE 2: MODERATE NEGATIVE (-1 < Score < 0)
        current_row += 2

        moderate_headers = ["Product", "Campaign Name", "C.P.P", "B.E", "Amount Spent (USD)", 
                           "Score", "Total Dates", "Days Checked", 
                           "Days with Negative Score", "Negative Score Dates", "Reason"]

        safe_write(negative_score_sheet, current_row, 0, 
                  "CAMPAIGNS WITH MODERATE NEGATIVE SCORE (-1 < SCORE < 0)", 
        ...
        if moderate_negative_campaigns:
            for campaign in moderate_negative_campaigns:
                ...
                safe_write(negative_score_sheet, current_row, 5, campaign['Total Score'], 
                          moderate_negative_data_format)
                safe_write(negative_score_sheet, current_row, 6, campaign['Total Dates'], 
                          moderate_negative_data_format)
                safe_write(negative_score_sheet, current_row, 7, campaign['Days Checked'], 
                          moderate_negative_data_format)
                safe_write(negative_score_sheet, current_row, 8, 
                          campaign['Days with Negative Score'], moderate_negative_data_format)
                safe_write(negative_score_sheet, current_row, 9, 
                          campaign['Negative Score Dates'], moderate_negative_data_format)
                safe_write(negative_score_sheet, current_row, 10, campaign['Reason'], 
                          moderate_negative_data_format)
                current_row += 1
        else:
            safe_write(negative_score_sheet, current_row, 0, 
                      "No campaigns found with moderate negative score (-1 < Score < 0)", 
                      moderate_negative_data_format)
            current_row += 1

        # Summary for moderate
        current_row += 2
        safe_write(negative_score_sheet, current_row, 0, "SUMMARY - MODERATE NEGATIVE CAMPAIGNS", 
                  moderate_negative_header_format)
        safe_write(negative_score_sheet, current_row + 1, 0, 
                  f"Campaigns with moderate negative score (-1 < Score < 0): {len(moderate_negative_campaigns)}", 
                  moderate_negative_data_format)

        # TABLE 3: POSITIVE (Score ≥ 0)
        current_row += 5

        positive_headers = ["Product", "Campaign Name", "C.P.P", "B.E", "Amount Spent (USD)", 
                           "Score", "Total Dates", "Days Checked", 
                           "Days with Negative Score", "Negative Score Dates", "Reason"]

        safe_write(negative_score_sheet, current_row, 0, 
                  "CAMPAIGNS WITH POSITIVE SCORE (SCORE ≥ 0)", positive_header_format)
        current_row += 1
        ...
        if positive_campaigns:
            for campaign in positive_campaigns:
                ...
                safe_write(negative_score_sheet, current_row, 7, campaign['Days Checked'], 
                          positive_data_format)
                safe_write(negative_score_sheet, current_row, 8, 
                          campaign['Days with Negative Score'], positive_data_format)
                safe_write(negative_score_sheet, current_row, 9, 
                          campaign['Negative Score Dates'], positive_data_format)
                safe_write(negative_score_sheet, current_row, 10, campaign['Reason'], 
                          positive_data_format)
                current_row += 1
        else:
            safe_write(negative_score_sheet, current_row, 0, 
                      "No campaigns found with positive score (Score ≥ 0)", 
                      positive_data_format)
            current_row += 1

        # Summary for positive
        current_row += 2
        safe_write(negative_score_sheet, current_row, 0, "SUMMARY - POSITIVE CAMPAIGNS", 
                  positive_header_format)
        safe_write(negative_score_sheet, current_row + 1, 0, 
                  f"Campaigns with positive score (Score ≥ 0): {len(positive_campaigns)}", 
                  positive_data_format)
        # Summary for positive
        current_row += 2
        safe_write(negative_score_sheet, current_row, 0, "SUMMARY - POSITIVE CAMPAIGNS", 
        ...
        safe_write(negative_score_sheet, current_row, 0, 
                  "CAMPAIGNS WITH NEGATIVE SCORE ON LAST DATE", last_date_header_format)
        current_row += 1

        last_date_headers = ["Product", "Campaign Name", "C.P.P", "B.E", "Amount Spent (USD)", 
                            "Score", "Last Date", "Last Date Score", 
        ...
        if last_date:
            ...
            for campaign_complete in all_campaigns_complete_analysis:
                ...
                # Get the last date's score from the day_wise_metrics
                last_date_metrics = campaign_complete['day_wise_metrics'].get(last_date, {})
                last_date_score = last_date_metrics.get('score', 0)
                
                # Only include campaigns with negative score on last date
                if last_date_score < 0:
                    ...
                    last_date_campaign = {
                        ...
                        'B.E': campaign_complete['B.E'],
                        'Amount Spent (USD)': round(total_amount_spent_usd, 2),
                        'Score': campaign_complete['Total Score'],
                        'Last Date': format_date_readable(last_date),
                        'Last Date Score': round(last_date_score, 2),
                        'Last Date Amount Spent (USD)': last_date_amount_spent,
                        'Last Date Purchases': last_date_purchases,
                        'Reason': f"Negative score ({round(last_date_score, 2)}) on last date ({format_date_readable(last_date)})"
                    }
                    
                    last_date_negative_campaigns.append(last_date_campaign)

        # Sort last date campaigns by last date score
        ...
        if last_date_negative_campaigns:
            for campaign in last_date_negative_campaigns:
                ...
                safe_write(negative_score_sheet, current_row, 4, campaign['Amount Spent (USD)'], 
                          last_date_data_format)
                safe_write(negative_score_sheet, current_row, 5, campaign['Score'], 
                          last_date_data_format)
                safe_write(negative_score_sheet, current_row, 6, campaign['Last Date'], 
                          last_date_data_format)
                safe_write(negative_score_sheet, current_row, 7, campaign['Last Date Score'], 
                          last_date_data_format)
                safe_write(negative_score_sheet, current_row, 8, 
                          campaign['Last Date Amount Spent (USD)'], last_date_data_format)
                safe_write(negative_score_sheet, current_row, 9, campaign['Last Date Purchases'], 
                          last_date_data_format)
                safe_write(negative_score_sheet, current_row, 10, campaign['Reason'], 
                          last_date_data_format)
                current_row += 1
            ...
            safe_write(negative_score_sheet, current_row, 0, 
                      f"No campaigns found with negative score on last date ({format_date_readable(last_date) if last_date else 'N/A'})", 
                      last_date_data_format)
            current_row += 1

        # Summary for last date
        current_row += 2
        safe_write(negative_score_sheet, current_row, 0, "SUMMARY - LAST DATE TABLE", 
                  last_date_header_format)
        safe_write(negative_score_sheet, current_row + 1, 0, 
                  f"Last date analyzed: {format_date_readable(last_date) if last_date else 'N/A'}", 
                  last_date_data_format)
        safe_write(negative_score_sheet, current_row + 2, 0, 
                  f"Campaigns with negative score on last date: {len(last_date_negative_campaigns)}", 
                  last_date_data_format)
        safe_write(negative_score_sheet, current_row + 3, 0, 
                  "Note: Campaigns already in Tables 1-3 are excluded from this table", 
                  last_date_data_format)
        
        # OVERALL SUMMARY
        current_row += 5
        safe_write(negative_score_sheet, current_row, 0, "OVERALL SUMMARY", 
                  severe_negative_header_format)
        ...
        safe_write(negative_score_sheet, current_row + 1, 0, 
                  f"Total campaigns analyzed: {total_campaigns}", severe_negative_data_format)
        safe_write(negative_score_sheet, current_row + 2, 0, 
                  f"Total campaigns meeting threshold (≥{selected_days} negative days): {len(filtered_campaigns)}", 
                  severe_negative_data_format)
        safe_write(negative_score_sheet, current_row + 3, 0, 
                  f"Severe negative campaigns (Score ≤ -1): {len(severe_negative_campaigns)}", 
                  severe_negative_data_format)
        safe_write(negative_score_sheet, current_row + 4, 0, 
                  f"Moderate negative campaigns (-1 < Score < 0): {len(moderate_negative_campaigns)}", 
                  moderate_negative_data_format)
        safe_write(negative_score_sheet, current_row + 5, 0, 
                  f"Positive campaigns (Score ≥ 0): {len(positive_campaigns)}", 
                  positive_data_format)
        safe_write(negative_score_sheet, current_row + 6, 0, 
                  f"Last date negative campaigns: {len(last_date_negative_campaigns)}", 
                  last_date_data_format)
        safe_write(negative_score_sheet, current_row + 7, 0, 
                  f"Days threshold used: {selected_days} out of {len(unique_dates)} total unique dates", 
                  severe_negative_data_format)
        safe_write(negative_score_sheet, current_row + 8, 0, 
                  f"Date range analyzed: {min(unique_dates)} to {max(unique_dates)}" 
                  if unique_dates else "No dates found", severe_negative_data_format)
        # OVERALL SUMMARY
        current_row += 5
        safe_write(negative_score_sheet, current_row, 0, "OVERALL SUMMARY", 
                  severe_negative_header_format)
        ...
        negative_score_sheet.set_column(0, 0, 25)   # Product
        negative_score_sheet.set_column(1, 1, 40)   # Campaign Name
        negative_score_sheet.set_column(2, 2, 15)   # C.P.P
        negative_score_sheet.set_column(3, 3, 15)   # B.E
        negative_score_sheet.set_column(4, 4, 25)   # Amount Spent (USD)
        negative_score_sheet.set_column(5, 5, 15)   # Score
        negative_score_sheet.set_column(6, 6, 20)   # Last Day Delivery Status / Total Dates
        negative_score_sheet.set_column(7, 7, 40)   # Comment / Days Checked
        negative_score_sheet.set_column(8, 8, 15)   # Total Dates / Days with Negative Score
        negative_score_sheet.set_column(9, 9, 15)   # Days Checked / Negative Score Dates
        negative_score_sheet.set_column(10, 10, 25) # Days with Negative Score / Reason
        negative_score_sheet.set_column(11, 11, 50) # Negative Score Dates
        negative_score_sheet.set_column(12, 12, 60) # Reason
        
        # ==== NEW SHEET: Scalable Campaigns (Based on Score) ====
        ...
        # Build a lookup of Score values using DAY-BY-DAY calculation (matching Negative Score Campaigns)
        campaign_score_lookup = {}
        ...
        for product, product_df in valid_products:
            for campaign_name, campaign_group in product_df.groupby("Campaign Name"):
                ...
                # Calculate overall Score using day-by-day method (matching Negative Score Campaigns)
                overall_score = 0
                total_net_profit_sum = 0
                ...
                if product_avg_price > 0:
                    ...
                    numerator_total = round(total_net_profit_sum, 2)
                    denominator_total = round(product_avg_price * calc_purchases_total * delivery_rate_total, 2)
                    net_profit_percentage = round((numerator_total / denominator_total * 100), 2) if denominator_total > 0 else 0
                    
                    # Score = Net Profit % / 10
                    overall_score = round(net_profit_percentage / 10, 2)
                
                # Calculate CPP and BE
                cpp = 0
                if total_amount_spent_usd > 0 and total_purchases == 0:
                    cpp = total_amount_spent_usd / 1
                elif total_purchases > 0:
        ...
        for campaign_key, campaign_data in campaign_score_lookup.items():
            if campaign_data['score'] > 0:
                scalable_campaign = {
                    'Product': campaign_key[0],
                    'Campaign Name': campaign_key[1],
                    'C.P.P': campaign_data['cpp'],
                    'B.E': campaign_data['be'],
        ...
        # Split into FOUR groups based on Score AND Amount Spent
        moderate_scalable_high_spend = [c for c in scalable_campaigns if 1 < c['Score'] <= 2 and c['Total Amount Spent (USD)'] >= 10]
        moderate_scalable_low_spend = [c for c in scalable_campaigns if 1 < c['Score'] <= 2 and c['Total Amount Spent (USD)'] < 10]
        high_scalable_high_spend = [c for c in scalable_campaigns if c['Score'] > 2 and c['Total Amount Spent (USD)'] >= 10]
        high_scalable_low_spend = [c for c in scalable_campaigns if c['Score'] > 2 and c['Total Amount Spent (USD)'] < 10]
        
        # Sort all four groups by Score (highest first)
        moderate_scalable_high_spend.sort(key=lambda x: x['Score'], reverse=True)
        moderate_scalable_low_spend.sort(key=lambda x: x['Score'], reverse=True)
        high_scalable_high_spend.sort(key=lambda x: x['Score'], reverse=True)
        high_scalable_low_spend.sort(key=lambda x: x['Score'], reverse=True)
        
        # ==== TABLE 1A: MODERATE SCALABLE - HIGH SPEND (Amount >= $10) ====
        safe_write(scalable_sheet, current_row, 0, "MODERATE SCALABLE CAMPAIGNS (1 < SCORE ≤ 2) - AMOUNT SPENT ≥ $10", moderate_scalable_header_format)
        current_row += 1
        ...
        if moderate_scalable_high_spend:
            for campaign in moderate_scalable_high_spend:
                ...
                safe_write(scalable_sheet, current_row, 2, campaign['C.P.P'], moderate_scalable_data_format)
                safe_write(scalable_sheet, current_row, 3, campaign['B.E'], moderate_scalable_data_format)
        ...
        avg_moderate_high_score = sum([c['Score'] for c in moderate_scalable_high_spend]) / total_moderate_high_campaigns if total_moderate_high_campaigns > 0 else 0
        ...
        safe_write(scalable_sheet, current_row + 3, 0, f"Average Score: {round(avg_moderate_high_score, 2)}", moderate_scalable_data_format)
        ...
        avg_moderate_low_score = sum([c['Score'] for c in moderate_scalable_low_spend]) / total_moderate_low_campaigns if total_moderate_low_campaigns > 0 else 0
        ...
        safe_write(scalable_sheet, current_row + 3, 0, f"Average Score: {round(avg_moderate_low_score, 2)}", moderate_scalable_data_format)
        
        current_row += 7
        
        # ==== TABLE 2A: HIGH SCALABLE - HIGH SPEND (Amount >= $10) ====
        safe_write(scalable_sheet, current_row, 0, "HIGH SCALABLE CAMPAIGNS (SCORE > 2) - AMOUNT SPENT ≥ $10", high_scalable_header_format)
        current_row += 1
        ...
        if high_scalable_high_spend:
            for campaign in high_scalable_high_spend:
                ...
                safe_write(scalable_sheet, current_row, 2, campaign['C.P.P'], high_scalable_data_format)
                safe_write(scalable_sheet, current_row, 3, campaign['B.E'], high_scalable_data_format)
        ...
        avg_high_high_score = sum([c['Score'] for c in high_scalable_high_spend]) / total_high_high_campaigns if total_high_high_campaigns > 0 else 0
        ...
        safe_write(scalable_sheet, current_row + 3, 0, f"Average Score: {round(avg_high_high_score, 2)}", high_scalable_data_format)
        ...
        avg_high_low_score = sum([c['Score'] for c in high_scalable_low_spend]) / total_high_low_campaigns if total_high_low_campaigns > 0 else 0
        ...
        safe_write(scalable_sheet, current_row + 3, 0, f"Average Score: {round(avg_high_low_score, 2)}", high_scalable_data_format)
        ...
        safe_write(scalable_sheet, current_row, 0, f"Total Scalable Campaigns (Score > 0): {total_scalable}", moderate_scalable_data_format)
        safe_write(scalable_sheet, current_row + 1, 0, f"  • Moderate (1 < Score ≤ 2): {total_moderate_campaigns}", moderate_scalable_data_format)
        ...
        safe_write(scalable_sheet, current_row + 4, 0, f"  • High (Score > 2): {total_high_campaigns}", high_scalable_data_format)
        ...
        scalable_sheet.set_column(2, 2, 15)  # C.P.P
        scalable_sheet.set_column(3, 3, 15)  # B.E
